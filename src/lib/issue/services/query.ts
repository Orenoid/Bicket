...
export async function getIssues(
  ...
  if (hasFilters) {
    ...
    for (const filter of filters) {
      ...
      if (
        filter.propertyType === PropertyType.MULTI_SELECT ||
        filter.propertyType === PropertyType.MINERS
      ) {
  ...
  const query = `
                ...
                WHERE 1=1 ${
                  hasFilters
                    ? `AND issue.id IN (${Array.from(filteredIssueIDs)
                        .map((id) => `'${id}'`)
                        .join(",")})`
                    : ""
                }
  ...
  // 查询属性值和构建返回数据
  const issuesWithValues = await getIssuesWithValues(
    issues.map((issue) => issue.id),
  );
  return { issues: issuesWithValues, total };
}

export async function getIssuesWithValues(
  issueIDs: string[],
): Promise<Issue[]> {
  // 并行查询工单的单值属性和多值属性
  ...
  // 按工单ID分组数据
  const singleValuesByIssueId = new Map<
    string,
    Array<(typeof singlePropertyValues)[0]>
  >();
  const multiValuesByIssueId = new Map<
    string,
    Array<(typeof multiPropertyValues)[0]>
  >();
 ...
 */
export function buildOrderByParams(
  ...
  const orderExpressions = sortConfigs.map((sort) => {
    ...
    const selectColumn = NUMBER_VALUE_TYPES.includes(propertyType)
      ? 'psv."number_value"'
      : 'psv."value"';
