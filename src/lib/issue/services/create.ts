...
export async function batchCreateIssues(
  ...
  try {
    const results = await prisma.$transaction(async (tx) => {
      ...
      const issueCreatePromises = inputs.map((input) =>
        tx.issue.create({
          data: {
      ...
      for (let i = 0; i < inputs.length; i++) {
        ...
        for (const pv of input.propertyValues) {
          ...
          const dbData = processor.transformToDbFormat(
            property,
            pv.value,
            issueId,
          );
...
function preprocessIssueInput(
  ...
  const containsReadonlyProperty = input.propertyValues.some((pv) =>
    readonlyPropertyIds.includes(pv.property_id),
  );
  ...
  for (const pv of input.propertyValues) {
    ...
    try {
      ...
      // 验证业务规则
      const businessResult = processor.validateBusinessRules(
        property,
        pv.value,
      );
      ...
      validationErrors.push(
        `属性 ${property.name} 处理器错误: ${(error as Error).message}`,
      );
      continue;
    }
  }
