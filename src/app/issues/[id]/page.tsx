...
import { getIssueById as getIssueByID, getPropertyDefinitions } from "@/lib/issue/services/query";
...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
    ...
    const mustFindProperty = (propertyID: string) => {
        const property = propertyDefinitions.find(property => property.id === propertyID);
        if (!property) {
    ...
    const getPropertyValue = (propertyID: string) => {
        const propertyValue = issue.property_values.find(propertyValue => propertyValue.property_id === propertyID);
        if (!propertyValue) {
            return null
        }
