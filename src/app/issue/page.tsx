...
// 页面主组件
export default async function Page({ searchParams }: { searchParams: Promise<{ filters?: string, page?: string, perPage?: string, sort?: string }> }) {
    try {
        // 从 URL 参数获取筛选条件和分页信息
        ...
        // 获取排序参数
        const sortStr = resolvedParams.sort;
        const sortConfigs = sortStr ? deserializeSort(sortStr) : [];

        // 构建排序参数
        const orderBy = buildOrderByParams(sortConfigs);
...
async function getIssuesWithValues(issues: { id: string }[], total: number): Promise<{ issues: Issue[], total: number }> {
    // 批量查询所有工单的单值属性
 ...
 */
function buildOrderByParams(sortConfigs: SortConfig[]): string | undefined {
