...
export async function createIssueAction(prevState: { message: string }, data: z.infer<typeof createIssueFormSchema>): Promise<ActionResult> {
    try {
...
export async function deleteIssueAction(issueID: string): Promise<ActionResult> {
    try {
        ...
        if (!isMember) {
            console.warn(`User ${userId} does not have permission to delete issue ${issueID}`);
            return {
                success: false,
                // 防止权限漏洞，统一告知 issue 不存在
                message: 'Issue does not exist.'
