...
export const UserAppliedFilter: AppliedFilterComponent = ({ filter }) => {
  ...
  const displayState = () => {
    ...
    if (isLoading) {
      return (
        <div className="flex items-center whitespace-nowrap">Loading...</div>
      );
    }
    ...
    if (userIds.length === 1) {
      ...
      return (
        <div className="flex items-center gap-x-2 whitespace-nowrap">
          ...
          <span className="text-xs text-gray-500">+{userIds.length - 2}</span>
        </div>
      );
    }
  };
