...
export const UserField: DetailFieldComponent = ({
    ...
    const [selectedUserID, setSelectedUserID] = useState<string>(typeof value === 'string' ? value : '');
    const hasValue = selectedUserID !== '';
    const [showClear, setShowClear] = useState(false); // shadcn select 不提供清除按钮，额外加上

    const { setPropertyValue, removePropertyValue, isLoading: isUpdating } = usePropertyUpdate(issueID);
    ...
    const renderUserItem = (user: NonNullable<typeof availableUsers>[number]) => (
        <div className="flex items-center">
            <Image
                src={user.imageUrl} alt={`${user.firstName || ''} ${user.lastName || ''}`}
    ...
    return (
        <div className="flex items-center">
            ...
            <div className="relative w-auto min-w-[120px] max-w-[240px] pl-3">
                <div className="relative w-full" onMouseEnter={() => setShowClear(true)} onMouseLeave={() => setShowClear(false)}>
                    {
                                    ...
                                    {availableUsers.length > 0 ? (
                                        <>
                                            {availableUsers.map(user => (
                                                <SelectItem key={user.userId || user.identifier} value={user.userId || ''}>
                                                    {renderUserItem(user)}
                                                </SelectItem>
                                            ))}
                                            {memberships?.hasNextPage && (
                                                <div className="px-4 py-2 text-center text-sm text-blue-600 hover:bg-gray-50 cursor-pointer" onClick={handleLoadMore}>
