...
export const MinersField: DetailFieldComponent = ({
  ...
  // 获取已选中的矿机信息
  const selectedMinersInfo = (
    isDropdownOpen ? tempSelectedMiners : selectedMiners
  )
  ...
  const handleRemoveMiner = (minerId: string, e: React.MouseEvent) => {
    ...
    if (isDropdownOpen) {
      ...
      // 标记有变更
      setHasChanges(
        JSON.stringify(newValues) !== JSON.stringify(selectedMiners),
      );
    } else {
      // 下拉框关闭时，直接执行更新
  ...
  return (
    <div className="flex items-center">
      ...
      <div
        className="relative w-auto min-w-[120px] max-w-[320px] pl-3"
        ref={dropdownRef}
      >
        {/* 触发下拉框的按钮/显示区域 */}
        <div
                  ...
                  (miner) =>
                    miner && (
                      ...
                      >
                        <span
                          className={cn(
                            "inline-block w-2 h-2 rounded-full mr-1 flex-shrink-0",
                            getMinerStatusStyle(miner.status),
                          )}
                        ></span>
                ...
                miners.map((miner) => (
                  ...
                  >
                    <div className="flex items-center flex-1 min-w-0">
                      <span
                        className={cn(
                          "inline-block w-3 h-3 rounded-full mr-2 flex-shrink-0",
                          getMinerStatusStyle(miner.status),
                        )}
                      ></span>
